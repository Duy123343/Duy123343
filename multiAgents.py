    ...
    def getAction(self, gameState: GameState):
        ...
        def maxValue(state, depth, agentIndex, alpha, beta):
            ...
            for action in state.getLegalActions(agentIndex):
                ...
                if nextAgent == 0:
                    v = max(v, maxValue(successor, newDepth, 0, alpha, beta))
                else:
                    v = max(v, minValue(successor, newDepth, nextAgent, alpha, beta))
        ...
        def minValue(state, depth, agentIndex, alpha, beta):
            ...
            for action in state.getLegalActions(agentIndex):
                ...
                if nextAgent == 0:
                    v = min(v, maxValue(successor, newDepth, 0, alpha, beta))
                else:
                    v = min(v, minValue(successor, newDepth, nextAgent, alpha, beta))
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState: GameState):
        ...
        def expectimaxValue(state, depth, agentIndex):
            ...
            if agentIndex == 0:
                ...
                for action in legalActions:
                    ...
                    best = max(best, expectimaxValue(succ, newDepth, nextAgent))
                return best
            else:
                total = 0.0
                ...
                for action in legalActions:
                    succ = state.generateSuccessor(agentIndex, action)
                    total += prob * expectimaxValue(succ, newDepth, nextAgent)
                return total
        ...
        for action in gameState.getLegalActions(0):
            successor = gameState.generateSuccessor(0, action)
            value = expectimaxValue(successor, self.depth, 1)
            if value > bestValue:
                bestValue = value
                bestAction = action
